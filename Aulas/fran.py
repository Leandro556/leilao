...
G = nx.Graph()  # Criação do Grafo - Cria um objeto de grafo não-direcionado usando a biblioteca NetworkX

names = ["A", "B", "C", "D", "E", "F"] #*Adição de Vértices - Adiciona 6 vértices nomeados de A a F*

for letter in names:
    G.add_node(letter)

# Add edges to create the graph structure - Adição de Arestas  - Conecta os vértices conforme a estrutura do grafo mostrada.*
G.add_edge("A", "B")
G.add_edge("A", "C")
G.add_edge("B", "C")
G.add_edge("B", "D")
...
# Display basic graph properties  -  Análise das Propriedades do Grafo - Mostra os vértices e um caminho gerado por busca em profundidade (DFS).
print("Chain:", list(G.nodes))
print("Path:", list(nx.dfs_edges(G)))

# Verificação de Ciclo - Tenta encontrar um ciclo no grafo (que existe, como A-B-C-A).
try: 
...
# Matriz de Incidência - Cria uma matriz que mostra quais arestas incidem em quais vértices.
incidence = nx.incidence_matrix(G).todense()
print("Incidence Matrix:\n", incidence)


# Propriedades Adicionais - Mostra o número de vértices, grau de cada vértice e o maior grau do grafo.
...
# Eulerian path utility functions - Algoritmos para Caminho Euleriano - Verifica se o grafo é conexo e tem 0 ou 2 vértices de grau ímpar (condição para existência de caminho euleriano).
...
# Verifica se uma aresta é uma "ponte" (sua remoção desconecta o grafo).
...
#Algoritmo de Fleury Encontra o caminho euleriano evitando atravessar pontes desnecessariamente.
def fleury(graph):
    """
...
# Algoritmo de Hierholzer - Constrói o caminho euleriano encontrando e unindo ciclos no grafo.
...
# Main program execution - Execução Principal - Verifica se há caminho euleriano e, se existir, aplica ambos algoritmos para encontrá-lo.
